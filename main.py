...
model = edsr(
    scale = 4, num_filters = 256, num_res_blocks=32, res_block_scaling=0.1
)
...
train_data_generator=train_data_generator(
   DATA_DIR,'train', scale=4.0, batch_size=2
)

test_x,test_y=next(
   test_data_generator(
   DATA_DIR,'test', scale=4.0, batch_size=2, shuffle=False
   )
)
